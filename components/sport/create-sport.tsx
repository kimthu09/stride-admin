...
import { createSport } from "@/lib/api/sport/createSport";
...
const CreateSportDialog = ({ onAdded }: CreateSportProps) => {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof SportSchema>> = async (data) => {
    ...
    createSport({ ...data, image: imageUrl })
      .then(() => {
        onAdded();
        return toast({
          variant: "success",
          title: "Success",
          description: "Add sport successfully",
        });
      })
      .catch((err: AxiosError<ApiError>) => {
        toast({
          variant: "destructive",
          title: "Error",
          description: err.response?.data.message ?? "Add sport failed",
        });
      });
  };
  ...
  return (
    ...
    >
      ...
      <DialogContent className="xl:max-w-[720px] max-w-[630px] p-0 bg-white">
        <DialogHeader>
          <DialogTitle className="p-6 pb-0">Add Sport</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="p-6 flex flex-col gap-4 border-y-[1px]">
            ...
            <div>
              ...
              <ImageUpload onUpload={handleUpload} fileCount={1} />
              {file === undefined && (
