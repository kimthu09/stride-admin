  ...
  return (
    <div className="flex flex-col w-full gap-4">
      <div className="flex flex-row items-center gap-2">
        {isHasExtensionAction && <Skeleton className="w-24 h-10" />}
        {isHasFilter && <Skeleton className="w-16 h-10" />}
        {isHasSearch && <Skeleton className="w-full h-10 flex-1" />}
        {isHasChooseVisibleRow && <Skeleton className="w-24 h-10" />}
      </div>
      <div className="flex flex-col rounded-md border overflow-x-auto">
        {[...Array(numberRow + 1)].map((_, index) => (
          ...
          >
            {isHasCheckBox && <Skeleton key={0} className="w-6 h-7" />}
      ...
      <div className="flex items-center justify-end space-x-2 mb-2">
        <div className="flex-1">
          {isHasCheckBox && <Skeleton className="w-56 h-10" />}
        </div>
        {isHasPaging && (
